...
- **常時Open**（クローズしない）
- 気づきは `/issue-start` Phase 8 の最後に、ユーザー確認を挟んでコメントとして追記される（1気づき = 1コメント、末尾のチェックボックスは未チェックのまま）
- 溜まったコメントはユーザーが独立した改善Issueへ昇格させ、元コメント末尾の `- [ ] 昇格済み` にチェックを付けて `- [x] 昇格済み ✅ Issue #<番号> で対応` に編集する。チェックの有無が回収済み判定の正式な基準
